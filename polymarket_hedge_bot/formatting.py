...
def ua_reason(reason: str) -> str:
    translations = {
        ...
        "orderbook has no usable bid/ask levels": "в orderbook немає придатних bid/ask рівнів",
        "realistic NO limit order candidate": "можна ставити реалістичний лімітний ордер на NO",
        "reference NO price is outside limit-entry strategy range": "орієнтир ціни NO поза діапазоном стратегії для limit entry",
        "orderbook spread is too wide for a realistic limit entry": "spread в orderbook завеликий для реалістичного limit entry",
        "suggested limit price is outside strategy range": "запропонована limit price поза діапазоном стратегії",
        "best bid is already above max strategy price; lower maker bid is unlikely to fill soon": "best bid вже вище максимальної ціни стратегії; нижчий maker bid навряд чи швидко заповниться",
